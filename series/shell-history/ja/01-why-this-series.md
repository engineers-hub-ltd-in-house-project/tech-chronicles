...
1990年代後半、駆け出しのエンジニアだった私は、友人に勧められてLinuxの世界に足を踏み入れた。CDから起動し、パーティションを切り、パッケージを選択し、長い時間をかけてインストールが完了する。再起動してログインすると、画面にプロンプトが現れた。`bash-2.05$`――この無骨な文字列が、私のUNIX人生の出発点である。
...
この「透明さ」に最初の亀裂が入ったのは、職場でBSD環境に触れたときだ。ログインした瞬間、プロンプトの見た目が違う。`%`記号が出迎える。bashでは`for`だった構文が`foreach`になっている。コマンドの挙動が微妙に異なる。これがtcshとの出会いであり、「シェルは選べるものだ」と私が初めて認識した瞬間でもあった。
