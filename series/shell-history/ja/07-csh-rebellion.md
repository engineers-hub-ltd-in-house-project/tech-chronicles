...
2000年代初頭、私は職場のサーバルームで初めてBSD系のUNIXに触れた。

それまでの私はLinux――Slackware、そしてRed Hat――の世界にいた。ターミナルを開けばbashが立ち上がる。それが当たり前だった。ところが、その職場のサーバルームにはFreeBSDのワークステーションが並んでいた。ログインすると、プロンプトの雰囲気が違う。何気なく`for`ループを書こうとして、構文エラーが出た。
